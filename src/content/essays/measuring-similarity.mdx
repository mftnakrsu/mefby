...
*RAG from First Principles. Part 3 of the series.*
...
So why isn't this the end of the article? Because for text, Euclidean distance has an awkward sensitivity: it cares about **magnitude**, the *length* of the vectors, not just their direction. And vector length often tracks things we do not care about, like how *long* a passage is. A one-line note and a three-paragraph summary can be about the very same topic, yet their embeddings can come out with quite different magnitudes, which pushes their arrowheads far apart in space. Euclidean distance dutifully reports a large gap and concludes "not very similar," even though, in meaning, they nearly are. We need something less fooled by length.
...
| Cosine similarity | The angle between directions | about `-1` to `1` (often `0` to `1` for text) | No | The default for text embeddings, when only meaning, not length, should count |
