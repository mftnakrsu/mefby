...
*RAG from First Principles. Part 9 of the series.*
...
The problem: even a perfect retrieval returns chunks that are mostly padding. A chunk that genuinely contains the answer also contains four sentences that have nothing to do with this particular question. That noise is not harmless. It burns context budget you are paying for, and worse, it dilutes the signal: recall "lost in the middle" from Part 7, where a model's attention sags on information buried in a long, noisy context. Relevant chunks, full of irrelevant sentences, make the answer worse.
...
Run it on a query whose sharp match is a thin sentence, and watch the before/after of what the model actually receives:
