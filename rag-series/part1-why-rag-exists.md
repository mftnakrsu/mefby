...
series: "RAG from First Principles"
part: 1
description: "Powerful LLMs still answer confidently and wrong about your own data and recent events. This first part explains why — and introduces RAG, the pattern that fixes it — with no code and a clear mental model."
date: 2026-06-09
tags: ["RAG", "LLM", "AI", "retrieval", "embeddings"]
---
...
<figure>
  <img src="part1-knowledge-cutoff.svg" alt="A horizontal timeline. A solid bar of training data ends at a 'knowledge cutoff' marker; beyond it, a faded, hatched region labelled 'the model knows nothing here' contains events — yesterday's news, your product launch, this morning's incident — leading to a 'Today' marker far to the right." />
  <figcaption>Figure 1 — The model's knowledge is frozen at its training cutoff; everything after it is a blind spot, no matter how recent or important.</figcaption>
</figure>
...
<figure>
  <img src="part1-llm-vs-rag.svg" alt="Two paths side by side. Left: a question goes into an LLM and out comes a confident wrong answer flagged as a hallucination with no source. Right: the same question first retrieves a snippet from a knowledge base, that snippet plus the question become an augmented prompt, the LLM reads it, and out comes a grounded, correct answer with a cited source." />
  <figcaption>Figure 2 — Same question, same model. The only difference is whether the relevant evidence was retrieved and placed in the prompt before the model answered.</figcaption>
</figure>
...
<figure>
  <img src="part1-open-book-exam.svg" alt="A friendly illustration of two students at desks. On the left, a student with closed, stacked books and a question-mark thought bubble guesses from memory, labelled 'Vanilla LLM'. On the right, a student with an open book, finger on a highlighted passage, calmly writes the answer, labelled 'RAG', with small notes mapping flip-to-the-page to retrieval, open-beside-the-question to augmentation, and write-in-your-own-words to generation." />
  <figcaption>Figure 3 — RAG doesn't make the model smarter; it turns a closed-book exam into an open-book one.</figcaption>
</figure>
...
<figure>
  <iframe src="part1-rag-pipeline.html" title="Interactive animation: one document flowing through the full RAG pipeline — chunk, embed, store, retrieve, augment, generate — with play/pause and step-through controls" width="100%" height="680" style="border:0;border-radius:14px;overflow:hidden"></iframe>
  <figcaption>Figure 4 — The full RAG pipeline, animated. Use Play/Pause or step Next/Back through chunk → embed → store → retrieve → augment → generate. (If your reader can't embed HTML, open <code>part1-rag-pipeline.html</code> directly.)</figcaption>
</figure>
